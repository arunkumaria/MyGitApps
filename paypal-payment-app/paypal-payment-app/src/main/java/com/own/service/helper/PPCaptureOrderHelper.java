...
public class PPCaptureOrderHelper {
	...
	public HttpRequest prepareRequest(String txnReference, TransactionDto transactionDto) {
		...
		String newCaptureOrderUrl = captureOrderUrl.replace("{provider-reference}", transactionDto.getProviderReference());
	...
	public PPOrderResponse processResponse(ResponseEntity<String> response) throws Exception {
		...
		if(response.getStatusCode().is4xxClientError() || response.getStatusCode().is5xxServerError()) {
			...
			throw new PaypalProviderException(errorResponse.getErrorCode(), errorResponse.getErrorMessage(), HttpStatus.valueOf(response.getStatusCode().value()));
